...
{
    public class HashTableQuadraticProbing<K, D> : HashTable<K, D>
    {
        HashTableBucket[] table;
        int elementCount = 0;

        public HashTableQuadraticProbing(int numberOfElements)
        {
        ...
        public override bool Insert(K key, D data)
        {
            ...
            if (elementCount < tableSize / 2)
            {
                ...
                {
                    if (BucketEmpty(GetIndexWithModifier(key, i * i)))
                    {
                        table[GetIndexWithModifier(key, i * i)] = new HashTableBucket(key, data);
                        elementCount++;
                        return true;
                    }
                }
        ...
        private int IndexSearch(K key)
        {
            ...
            for (int i = 0; i < tableSize; i++)
            {
                int index = GetIndexWithModifier(key, i * i);
